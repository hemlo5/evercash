...
export function StatCard({ title, value, change, changeType, icon: Icon, contributors }: StatCardProps) {
  ...
  return (
    <div className="glass-card-hover p-6 rounded-2xl animate-fade-in-up h-full flex flex-col justify-between">
      <div>
        ...
        <div className={`mt-4 p-4 rounded-xl border border-accent/30 flex-1 flex flex-col justify-center min-h-[110px] ${
          ...
          <ul className="space-y-2">
            {contributors.map((c, idx) => (
              <li key={idx} className="flex justify-between text-base font-medium text-white">
                <span className="truncate max-w-[140px]">{c.name}</span>
                <span className="font-mono tabular-nums text-right">{c.value}</span>
              </li>
            ))}
          </ul>
        </div>
